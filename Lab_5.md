...
QUERY PLAN
`--SEARCH TABLE big_cards USING COVERING INDEX IDX2_big_cards (race=?)
Run Time: real 0.001 user 0.000000 sys 0.015625
```
...
Your answer:
```
From 5’s execution, the only difference of the time is after executing ‘VACUUM’ there the time has increased in the update. Otherwise for the most part, the update did not change the time without the indexes. It remained the same consistently besides 5’s execution.
```
...
Findings: Execution time changed for update after executing ‘VACUUM’. Everything seemed to be working just find in utilizing the indexes. Nothing out of the ordinaire didn’t function properly. Most of the execution run time: took a very small marginal number (most cases real 0.001) on the update executions.
